...
<h1 class="fw-bold mb-4 mt-3">Daftar work center</h1>
    ...
    <tbody>
      @foreach ($workCenters as $workCenter)
        <tr>
          ...
          <td id="{{ 'code' . $workCenter->id }}" class="text-capitalize job-type-code">{{ $workCenter->code }}</td>
          <td id="{{ 'desc' . $workCenter->id }}">{{ $workCenter->description }}</td>
          <td>
            <div>
              <span class="badge bg-warning my-hover">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16" onclick="detailWorkCenter({{ $workCenter->id }})">
                  <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                </svg>
              </span>
              
              <form action="{{ route('work-center.destroy', $workCenter->id) }}" method="post" class="d-inline-block">
                @csrf
                @method('DELETE')
  ...
  <div class="modal" id="modal" tabindex="-1">
    <div class="modal-dialog">
      <div class="modal-content my-bg-element">
        ...
        <form method="post" id="new-work-center" action="/work-center">

          <div class="modal-header">
            <h5 class="modal-title">Tambah work center</h5>
          </div>
...
<script>
  ...
  $('#add').on('click', function(){
    ...
    const data = $('#new-work-center').serialize();
